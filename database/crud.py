...
def get_location(loc_id, db_session):
    return db_session.query(Location).filter(Location.data_id == loc_id).first()


# FUNCTION FOR GET SOCIAL
def get_social(social_id, db_session):
    return db_session.query(Social).filter(Social.data_id == social_id).first()
...
def get_job_data_by_name_id(job_name_id: int, db_session):
    job_model = db_session.query(Job).filter(Job.name_id == job_name_id).first()
